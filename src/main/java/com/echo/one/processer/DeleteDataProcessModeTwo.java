...
@Component(value = "process.deleteData.2")
public class DeleteDataProcessModeTwo extends ProcessMode {

    private static final Logger logger = LoggerFactory.getLogger(DeleteDataProcessModeTwo.class);

    @Override
    protected void handler(TransferContext transferContext) {
        String traceId = transferContext.getTraceId();
        logger.info("traceId: {}, 开始执行mode为2的删除process", traceId);
    }
}
