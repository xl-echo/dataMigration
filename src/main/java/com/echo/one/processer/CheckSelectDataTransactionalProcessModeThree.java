...
@Component(value = "process.checkSelectData.3")
public class CheckSelectDataTransactionalProcessModeThree extends ProcessMode {

    private static final Logger logger = LoggerFactory.getLogger(CheckSelectDataTransactionalProcessModeThree.class);

    @Override
    protected void handler(TransferContext transferContext) {
        String traceId = transferContext.getTraceId();
        logger.info("traceId: {}, 开始执行mode为3的检查查询代码", traceId);
