...
@Component(value = "process.insertData.2")
public class InsertDataProcessModeTwo extends ProcessMode {

    private static final Logger logger = LoggerFactory.getLogger(InsertDataProcessModeTwo.class);

    @Override
    protected void handler(TransferContext transferContext) {
        String traceId = transferContext.getTraceId();
        logger.info("traceId: {}, 开始执行mode为2的插入process", traceId);
    }
